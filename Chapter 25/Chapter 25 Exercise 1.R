  ...
  for (i in 1:n) {
    ...
    text(pt$x + 1,
      ...
      label = paste("\"", cols[i],
        "\"", "\nR: ", rgbval[1], "  G: ",
        rgbval[2], "  B: ", rgbval[3], "\nhex: ",
        rgb(t(rgbval), maxColorValue = 255),
        sep = ""
      )
    )
  }
...
plot(1, 1,
  type = "n", xaxt = "n", yaxt = "n", ann = F, xlim = c(-10, 510), ylim = c(0.5, 2.5),
  ylab = c("testcolors", "gray.colors")
)
points(
  x = rep(1:500, 2), y = rep(1:2, each = 500),
...
plot(surv$Wr.Hnd, surv$Height,
  col = NW.cols2, pch = 19,
  xlab = "Writing handspan (cm)", ylab = "Height (cm)"
)
colorlegend(NW.pal(200),
  zlim = range(surv$NW.Hnd), zval = seq(13, 23, by = 2),
...
depth.cols <- rgb(depth.pal2(normalize(quakes$depth)),
  maxColorValue = 255,
  alpha = 0.6 * 255
)
plot(quakes$mag,
