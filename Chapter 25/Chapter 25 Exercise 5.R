...
persp(
  x = hp.seq, y = wt.seq, z = car.pred.mat, theta = 120,
  phi = 50
)

persp(
  x = hp.seq, y = wt.seq, z = car.pred.mat, theta = 0, phi = 23,
...
chor.out.mat <- matrix(!inside.owin(
  x = chor.xy$Var1,
  y = chor.xy$Var2,
  w = chor.WIN
))
...
z.mat1 <- matrix(
  predict(nuc.fit.1, # First linear model
    newdata = predictors
  ),
  nrow = 50,
  ncol = 50
...
persp(
  x = date.seq, # plot for the firs linear Model
  y = cap.seq, theta = 30,
  z = z.mat1,
...
facetmat <- (
  volcano[-1, -1]
  + volcano[-1, -ncol(volcano)] + volcano[-nrow(volcano), -1] + volcano[-nrow(volcano), -ncol(volcano)]
) / 4
