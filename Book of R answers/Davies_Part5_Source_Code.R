...
expr3 <- expression(paste("Sample mean:    ", italic(n)^{
  -1
}, sum(italic(x)[italic(i)], italic(i) == 1, italic(n)) == frac(italic(x)[1] + ... + italic(x)[italic(n)], italic(n))))
...
surv %>%
  ggvis(x = ~Wr.Hnd, y = ~Height, fill = filler, size := sizer, opacity := opacityer) %>%
  layer_points() %>%
...
filled.contour(x = quak.dens$x, y = quak.dens$y, z = quak.dens$z, color.palette = topo.colors, nlevels = 30, xlab = "Longitude", ylab = "Latitude", key.title = title(main = "KDE", cex.main = 0.8), plot.axes = {
  axis(1)
  axis(2)
  points(quakes$long, quakes$lat, cex = 0.5, col = adjustcolor("black", alpha = 0.3))
})
...
bgplot3d({
  plot.new()
  colorlegend(slen.pal(50), zlim = range(slen), zval = seq(4.5, 7.5, 0.5), digit = 1, posx = c(0.91, 0.93), posy = c(0.1, 0.9), main = "Sepal length")
})
...
bgplot3d({
  plot.new()
  colorlegend(col = rbow, zlim = range(chor.dens.WIN$z, na.rm = TRUE), zval = seq(0, 0.02, 0.0025), main = "KDE", digit = 4, posx = c(0.87, 0.9), posy = c(0.2, 0.8))
})
...
segments3d(
  x = c(rep(rs, each = 2, times = res), rep(c(ran[1], ran[2]), times = res^2), rep(rs, each = 2, times = res)),
