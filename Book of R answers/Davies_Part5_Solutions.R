...
Salaries %>%
  ggvis(x = ~salary, fill = ~rank) %>%
  group_by(rank) %>%
  layer_densities(adjust = input_slider(0.1, 2, label = "Smoothness")) %>%
  add_axis("x", title = "Salary") %>%
  add_axis("y", title = "Kernel estimate") %>%
...
filled.contour(faith.dens$x, faith.dens$y, faith.dens$z, color.palette = faith.cols, xlab = "Waiting time", ylab = "Eruption duration", plot.axes = {
  axis(1)
  axis(2)
  points(faithful$waiting, faithful$eruption, cex = 0.5, col = "gray")
})
# (h)
...
bgplot3d({
  plot.new()
  colorlegend(ozcols, zlim = range(aq$Temp), zval = seq(60, 95, 5), posx = c(0.91, 0.93), posy = c(0.1, 0.9))
})
